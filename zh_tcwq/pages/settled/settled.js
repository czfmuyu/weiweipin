...
Page({
    ...
    formSubmit: function(e) {
        ...
        console.log(t + " " + n);
        wx.getStorageSync("uniacid");
        var i = wx.getStorageSync("openid"), s = a.data.facilities, l = a.data.hotel_logo, c = a.data.wechat, u = a.data.yyzz, r = a.data.sfzzm, d = e.detail.value.business_name, p = e.detail.value.business_keyword, g = a.data.address, m = a.data.time, f = a.data.time1, h = e.detail.value.business_number, y = e.detail.value.business_notice, w = e.detail.value.vr_demo, x = e.detail.value.textarea, v = a.data.coordinates, _ = a.data.store, T = a.data.store2, S = a.data.index, D = a.data.index_two, b = _[S], z = T[D];
        ...
        var C = "";
        if ("" != x && null != x || (x = ""), console.log(a.data, u, r), "" == d ? C = "请输入商家名称" : "" == p ? C = "请输入关键字" : "" == g ? C = "请输入详细地址" : null == s ? C = "请勾选店内设施" : 0 == s.length ? C = "请勾选店内设施" : "00:00" == m ? C = "请输入营业开始时间" : "00:00" == f ? C = "请输入营业结束时间" : "" == h ? C = "请输入联系电话" : "" == y ? C = "请输入公告说明" : "1" == a.data.xtxx.is_img && null == u ? C = "请上传营业执照照片" : "1" == a.data.xtxx.is_img && null == r ? C = "请上传法人身份证正面照片" : 540 <= x.length ? C = "内容超出" : 0 == a.data.sms ? null == a.data.num && (C = "请进行手机号验证") : 1 == a.data.sms && null == a.data.yz_code && (C = "请进行手机号验证"), 
        "" != C) wx.showModal({
            title: "提示",
            content: C,
            showCancel: !0,
