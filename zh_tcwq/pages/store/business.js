...
Page({
    ...
    refresh: function(t) {
        var l = this, e = l.data.id, i = l.data.typeid, u = l.data.page, p = l.data.storelist, a = wx.getStorageSync("city");
        console.log("城市为" + a), console.log(e, i, p, u), app.util.request({
            ...
            success: function(t) {
                for (var e in l.setData({
                    page: u + 1
                }), console.log(t), t.data.length < 10 ? l.setData({
                    refresh_top: !0
                }) : l.setData({
                    refresh_top: !1
                }), t.data) {
                    ...
                    var a = Number(wx.getStorageSync("Location").latitude), o = Number(wx.getStorageSync("Location").longitude), c = i[0], n = i[1], r = a * Math.PI / 180, s = c * Math.PI / 180, g = r - s, d = o * Math.PI / 180 - n * Math.PI / 180, h = 2 * Math.asin(Math.sqrt(Math.pow(Math.sin(g / 2), 2) + Math.cos(r) * Math.cos(s) * Math.pow(Math.sin(d / 2), 2)));
                    h *= 6378.137;
                    h = (h = Math.round(1e4 * h) / 1e4).toFixed(2);
                    t.data[e].distance = h;
                }
                p = p.concat(t.data), l.setData({
                    store: p,
                    storelist: p,
                    fjstorelist: p
                });
