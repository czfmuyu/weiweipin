...
Page({
    ...
    formSubmit: function(e) {
        var t = this, o = wx.getStorageSync("users"), a = (o.id, o.openid), n = t.data.images, i = t.data.store, c = t.data.procedures, s = e.detail.value, l = Number(s.money), r = Number(s.share), u = s.details, d = t.data.checked1, g = t.data.checked2;
        if (0 == d) var f = 1, h = (l + c * l).toFixed(2), m = l / r; else f = 2, h = (l * r + l * r * c).toFixed(2);
        if (0 == g) var p = "", y = 1; else p = s.hb_keyword, y = 2;
        console.log(imgArray);
        var w = "";
        console.log(w), console.log("红包总金额  " + h);
        var x = new RegExp("^[一-龥]+$"), S = "";
        "" == u ? S = "福利描述不能为空" : "" == l ? S = "红包金额不能为空" : !t.data.checked1 && l < 1 ? S = "福利红包金额不能小于1元" : "" == r ? S = "红包个数不能为空" : m < .1 ? S = "红包份数过大，请合理设置" : t.data.checked1 && l < .1 ? S = "单个红包最小金额不能小于0.1" : 1 == g && ("" == p ? S = "红包口令不能为空" : x.test(p) || (S = "口令只能输入汉字")), 
        "" != S ? wx.showModal({
            title: "提示",
            content: S,
            success: function(e) {},
